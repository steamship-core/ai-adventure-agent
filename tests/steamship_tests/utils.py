...
from steamship import Configuration, EmbeddingIndex, Steamship, Task, Workspace
...
def get_steamship_client(fail_if_workspace_exists=False, **kwargs) -> Steamship:
    ...
    return Steamship(
        ...
        config=Configuration.parse_obj(kwargs),
    )
...
def random_name() -> str:
    ...
    return (
        f"test_{''.join(random.choice(letters) for _ in range(10))}".lower()
    )  # noqa: S311
...
def random_index(steamship: Steamship, plugin_instance: str) -> EmbeddingIndex:
    index = steamship.use_plugin(
        ...
        random_name(),
        config={
            "embedder": {"plugin_handle": plugin_instance, "fetch_if_exists": True}
        },
