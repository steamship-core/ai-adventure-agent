...
from utils.tags import QuestIdTag, TagKindExtensions, CharacterTag


class ChatHistoryFilter(ABC):

    @abstractmethod
    def filter_blocks(self, chat_history_file: File) -> List[Tuple[Block, Optional[str]]]:
        """ Returns a list of included blocks, and optional explanations of why included for debugging"""
        pass

    def filter_chat_history(self, chat_history_file: File, filter_for: Optional[str] = None) -> List[int]:
        ...
        print(f"{filter_for} input ************")
        for i, (block, inclusion_reason) in enumerate(filtered_blocks):
            print(
...
class UnionFilter(ChatHistoryFilter):
    ...
    def filter_blocks(self, chat_history_file: File) -> List[Tuple[Block, Optional[str]]]:
        ...
        for filter in self.filters:
            all_results.extend(filter.filter_blocks(chat_history_file=chat_history_file))
        return self.dedupe_results(all_results)

    def dedupe_results(self, input: List[Tuple[Block, Optional[str]]]) -> List[Tuple[Block, Optional[str]]]:
