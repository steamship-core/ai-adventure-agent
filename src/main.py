...
from schema import UserSettings, ServerSettings, Quest
...
class AdventureGame():
    ...
    def collect_user_settings(self) -> UserSettings:
        ...
        self.user_settings.name = input("What is your character's name? ")
        self.user_settings.background = input("What's your character's backstory? ")
        self.user_settings.inventory = input("What is your character's starting item? ")
        self.user_settings.inventory += "\n"
        self.user_settings.motivation = input("What is your character motivated to achieve? ")
    ...
    def embark_on_quest(self, quest: Quest):
        ...
        setting_description = self.llm.generate(quest.chat_history.file.id, options={"max_tokens":100}).wait().blocks[0].text[:1000]
        ...
        quest.chat_history.append_system_message(f"What problem did {self.user_settings.name} encounter next? ")
        problem = self.llm.generate(quest.chat_history.file.id).wait().blocks[0].text
        quest.chat_history.append_system_message(f"{self.user_settings.name} encountered a problem: {problem}")
        solution = input(f"\n*****\nOh no! {self.user_settings.name} has a problem: {problem}. \nWhat should {self.user_settings.name} do? ")
        quest.chat_history.append_system_message(f"{self.user_settings.name} solved that by {solution}. What happens next? ")
        ...
        quest.chat_history.append_system_message(f"How does this mission end? {self.user_settings.name} should not yet achieve their overall goal of {self.user_settings.motivation}")
        ...
        quest.chat_history.append_system_message(f"What object or item did {self.user_settings.name} find during that story? It should fit the setting of the story and help {self.user_settings.name} achieve their goal. Please respond only with ITEM NAME: <name> ITEM DESCRIPTION: <description>")
        item = self.llm.generate(quest.chat_history.file.id, options={"max_tokens":100}).wait().blocks[0].text
        ...
        quest.chat_history.append_system_message("Summarize this mission in three sentences.")
        summary = self.llm.generate(quest.chat_history.file.id, options={"max_tokens": 200}).wait().blocks[0].text
